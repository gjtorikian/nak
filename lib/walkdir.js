...
var fs = require("fs"),
    ...
    asyncForEach = function (arr, iterator, callback) {
        var completed = 0;
        arr.forEach(function (x, itr) {
            iterator(x, itr, function (err) {
                if (!err) {
                    callback(err);
                    callback = function () {};
                }
...
module.exports = function(fpath, options, cb) {
  ...
  finalizer = function() {
    if (options.list)
      ...
      /*for (var m = 0; m < matchedFileLength; m++) {
        ...
        //var matchCount = contents.match(options.query).length, 
        //    contents = contents.split("\n"), contentsLength = contents.length;

        /*asyncForEach(contents, function(line, itr, cb) {
          if (options.query.test(line)) {
            lines += currFile + ":" + (itr+1) + ":" + (options.replacement ? replacedContents[i] : line) + "\n";
            cb(--matchCount);
          }
          else
            cb(matchCount);
  ...
  readdir = function(path,depth){
    ...
    var readdirAction = function(files) {
      ...
      var fileCount;
      if( !(fileCount = files.length) ) return; // empty dir

      if (depth > 1)
        var filepath = _path.relative(fpath, path);

      for (var i = 0; i < fileCount; i++) {
        var filename = files[i], oFilename = filename;
        if (depth > 1)  
          filename = filepath + "/" + oFilename;

        if (options.filesInclude.test(oFilename) && !ignorer.isIgnored(options.exclusions, options.exclusionsLength, filename, options.hidden)) {
            statter(path + "/" + oFilename,(depth || 0)+1);
        }
      }
    };
  ...
  processFile = function(path, stat) {
