  ...
  callback = function() {
    if (options.list)
      ...
      for (var m = 0; m < matchedFileLength; m++) {
        ...
        var contents = allFiles[currFile].contents, matchCount = allFiles[currFile].matchCount, lines = "";
        ...
        for (var i = 0, strLength = contents.length; i < strLength && matchCount > 0; i++) {
    ...
    q.push({type: "query", path: path}, function (err, fileBuffer) {
      ...
      if (!isBinaryFile(fileBuffer, stat.size)) {
        ...
        if ( options.query.test(fileString) ) {
          ...
          allFiles[path].matchCount = fileString.match(options.query).length;
