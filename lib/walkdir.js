...
walker.walkdir = function(fpath, options, printCb, done) {
  ...
  ignorer = new Ignorer(options.exclusions);
  ...
  readdir = function(parent, path) {
    ...
    var readdirAction = function(files) {
      ...
      while (i < fileCount) {
        ...
        if (options.filesInclude.test(file) && !ignorer.isFileIgnored(_path.resolve(fpath, parent), file, filename, options.hidden)) {
          statter(path, filename);
        }
      }
    };

    if (!ignorer.isDirIgnored(_path.resolve(fpath, parent), path, options.hidden)) {
      job(1);
