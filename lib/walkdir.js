...
walker.walkdir = function(fpath, options, printCb, done) {
  ...
  readdir = function(startingPath, filepath) {
    ...
    var readdirAction = function(files) {
      ...
      var fileCount = files.length, i = 0, prefix = filepath + DIR_SEP, file;
      allPaths[filepath] = [];
    ...
    if (!ignorer.isDirIgnored(path.relative(fpath, filepath), filepath, filepath.split(DIR_SEP), options.hidden)) {
      job(1);
