...
var MMB = require("./minimatch_better").Minimatch;
...
Ignorer.prototype.isFileIgnored = function(name, path, splitPath, showHidden) {
  ...
  while (r < this.ignoresLength) {
    if (this.ignores[r++].match(name, path, splitPath))
      return true;
  }
  return false;
}

Ignorer.prototype.isDirIgnored = function(name, path, splitPath, showHidden) {
  if (!showHidden && (/\/\.\w/.test(name))) {
    return true;
  }
  ...
  var r = 0;
  while (r < this.ignoresLength) {
    if (this.ignores[r++].match(path, path, splitPath))
      return true;
  }
  return false;
}
