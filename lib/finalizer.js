            ...
            for (var i = 0, strLength = contentsSplit.length; matchCount; i++) {
              ...
              if (options.queryClean.test(matchLine)) {
                ...
                while ( options.query.test(matchLine) ) {
                  if (options.ackmate) {
                    if (options.replacement) {
                      matchLine.replace(options.queryClean, options.replacement);
                      lastMatchLength = options.replacement.length;
                      strPos.push(options.replacementRe.lastIndex - lastMatchLength + " " + lastMatchLength);
                    }
                    else {
                ...
                if (options.replacement)
                  matchLine = matchLine.replace(options.queryClean, options.replacement);
                if (options.ackmate)
